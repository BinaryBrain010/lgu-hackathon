...
export default function HODDashboard() {
  ...
  const pending = departmentClearances.filter((c) => c.status === "pending").length
  const approved = departmentClearances.filter((c) => c.status === "approved").length
  const inReview = departmentClearances.filter((c) => c.status === "in_review").length
        ...
        <Card className="p-4">
          <div className="flex items-start justify-between">
            <div>
              ...
              <p className="text-2xl font-bold text-gray-900">{departmentClearances.length}</p>
            </div>

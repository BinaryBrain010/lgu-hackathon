...
export default function AccountsDashboard() {
  ...
  const pending = accountsClearances.filter((c) => c.status === "pending").length
  const approved = accountsClearances.filter((c) => c.status === "approved").length
  const inReview = accountsClearances.filter((c) => c.status === "in_review").length
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <Card className="p-4">
          <div className="flex items-start justify-between">
            <div>
              ...
              <p className="text-2xl font-bold text-gray-900">{accountsClearances.length}</p>
            </div>
        ...
        <Card className="p-4">
          <div className="flex items-start justify-between">
            <div>
              <p className="text-xs text-gray-600 mb-1">Cleared</p>

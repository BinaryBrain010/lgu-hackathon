...
export class NotificationService extends BaseService<Notification, NotificationCreateInput, NotificationUpdateInput, NotificationWhereInput> {
  protected model = prisma.notification;

  async findByUserId(userId: string, unreadOnly: boolean = false): Promise<Notification[]> {
    const where: NotificationWhereInput = { userId, ...(unreadOnly && { read: false }) };
  ...
  async getUnreadCount(userId: string): Promise<number> {
    return await this.model.count({ where: { userId, read: false } });
  }

  protected buildWhereClause(params: FilterParams): NotificationWhereInput {
    const where: NotificationWhereInput = {};
    if (params.userId) where.userId = params.userId as string;
    if (params.read !== undefined) where.read = params.read === 'true' || params.read === true;
    if (params.search) {
      where.OR = [

...
export class ClearanceService extends BaseService<DegreeClearance, ClearanceCreateInput, ClearanceUpdateInput, ClearanceWhereInput> {
  ...
  async findByStudentId(studentId: string): Promise<DegreeClearance | null> {
    return await this.model.findUnique({
      ...
      include: {
        student: true,
        remarks: { include: { officer: true }, orderBy: { createdAt: 'desc' } },
      },
    });
  }
  ...
  async initiateClearance(studentId: string): Promise<DegreeClearance> {
    try {
      ...
      if (existing) throw new Error('Clearance already initiated');
  ...
  async approveDepartment(clearanceId: string, department: ClearanceDepartment, officerId: string, message?: string): Promise<DegreeClearance> {
    try {
      const clearance = await this.findOne(clearanceId);
      if (!clearance) throw new Error('Clearance not found');

      const updateData: any = {};
      if (department === ClearanceDepartment.DEPARTMENT) updateData.departmentStatus = ClearanceStatus.APPROVED;
      else if (department === ClearanceDepartment.ACADEMIC) updateData.academicStatus = ClearanceStatus.APPROVED;
      else if (department === ClearanceDepartment.STUDENT_AFFAIRS) updateData.affairsStatus = ClearanceStatus.APPROVED;
      else if (department === ClearanceDepartment.ACCOUNTS) updateData.accountsStatus = ClearanceStatus.APPROVED;

      if (message) {
        await prisma.clearanceRemark.create({ data: { clearanceId, department, message, officerId } });
      }
  ...
  async rejectDepartment(clearanceId: string, department: ClearanceDepartment, officerId: string, message: string): Promise<DegreeClearance> {
    try {
      const clearance = await this.findOne(clearanceId);
      if (!clearance) throw new Error('Clearance not found');

      const updateData: any = { status: ClearanceStatus.REJECTED };
      if (department === ClearanceDepartment.DEPARTMENT) updateData.departmentStatus = ClearanceStatus.REJECTED;
      else if (department === ClearanceDepartment.ACADEMIC) updateData.academicStatus = ClearanceStatus.REJECTED;
      else if (department === ClearanceDepartment.STUDENT_AFFAIRS) updateData.affairsStatus = ClearanceStatus.REJECTED;
      else if (department === ClearanceDepartment.ACCOUNTS) updateData.accountsStatus = ClearanceStatus.REJECTED;
  ...
  private async updateOverallStatus(clearanceId: string): Promise<void> {
    const clearance = await this.model.findUnique({ where: { id: clearanceId } });
    if (!clearance) return;
    ...
    if (allApproved) {
      await this.update(clearanceId, { status: ClearanceStatus.APPROVED, completedAt: new Date() });
      await prisma.notification.create({
        data: {
          userId: clearance.studentId,
      ...
      await this.update(clearanceId, { status: ClearanceStatus.IN_REVIEW });
    }
  }

  protected buildWhereClause(params: FilterParams): ClearanceWhereInput {
    const where: ClearanceWhereInput = {};
    if (params.studentId) where.studentId = params.studentId as string;
    if (params.status) where.status = params.status as ClearanceStatus;
    return where;
  }
}

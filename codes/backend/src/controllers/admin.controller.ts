...
import { Role, FYPStage, ClearanceStatus } from '@prisma/client';
...
export class AdminController {
  ...
  createUser = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
    try {
      // This would need validation in a real app
  ...
  private async getFYPStageDistribution(): Promise<Record<string, number>> {
    const stages = await prisma.fYP.groupBy({
      by: ['stage'],
      _count: {
        stage: true,
      },
  ...
  private async getClearanceStatusDistribution(): Promise<Record<string, number>> {
    const statuses = await prisma.degreeClearance.groupBy({
      by: ['status'],
      _count: {
        status: true,
      },

...
export class FYPController extends BaseController<any, any, any, any> {
  ...
  uploadDocument = async (req: AuthRequest, res: Response, next: NextFunction): Promise<void> => {
    try {
      ...
      // Update FYP stage based on document type
    ...
    try {
      ...
      if (!canTransition) {
        ApiResponseHandler.badRequest(
          res,
          `Cannot transition from ${fyp.stage} to ${validatedData.stage}`
        );
        return;
      }
  ...
  uploadPlagiarismReport = async (
    ...
    try {
      ...
      if (validatedData.similarity > PLAGIARISM.THRESHOLD) {
        ApiResponseHandler.badRequest(
          res,
          `Plagiarism similarity (${validatedData.similarity}%) exceeds threshold (${PLAGIARISM.THRESHOLD}%)`
        );
        return;
      }
